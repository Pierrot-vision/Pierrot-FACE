...
- 2026-09-02 — 🔬 **추론 단계 축 8개를 전부 기각** — 남은 차이는 후처리가 아니라 가중치 안에
  있습니다 👉 [Phase 10](LAB/FA3D/Exp/Phase_10_추론축_소진.md)
...
- 2026-09-01 — ⚠ **AFLW 격차의 정체는 자세가 아니라 가림** — 처방은 실패했고 용량 반응만
  남았습니다 👉 [Phase 8](LAB/FA3D/Exp/Phase_8_AFLW_가림.md)
- 2026-08-31 — ⚠⚠ **NME 는 형상을 거의 안 잽니다** — 형상을 GT 로 완벽히 바꿔도 1.3% 밖에
  나아지지 않습니다. 그래서 형상 정확도를 따로 잽니다 👉 [Phase 6](LAB/FA3D/Exp/Phase_6_지표가_형상을_안_잰다.md)
- 2026-08-31 — 🔍 **남은 격차는 최적화가 아니라 일반화** — 학습 도메인에서는 우리가
  이깁니다 👉 [Phase 4](LAB/FA3D/Exp/Phase_4_일반화_격차_진단.md)
...
한 장의 2D 사진에서 **62개의 숫자**를 회귀하고, 그것을 BFM 3DMM 기저로 펴서
**38,365 정점 · 76,073 삼각형**의 3D 얼굴과 68 랜드마크를 만듭니다.

```
p[0:12]   T = f[R; t3d]   상사변환 (Euler 각 대신 행렬 회귀 — gimbal lock 회피)
p[12:52]  alpha_shp (40)  BFM shape PCA 계수 — "누구인가"
p[52:62]  alpha_exp (10)  표정 PCA 계수     — "지금 어떤 표정인가"
```
...
왼쪽 위에는 검출된 얼굴별 **3D 메쉬 크롭**이 세로로 쌓이고, 사람마다 색이 다릅니다 —
추적 ID 로 고정돼 프레임이 바뀌어도 같은 색을 유지합니다.
전체 영상(95초·H.264)은 [`docs/FA3D/demo_full.mp4`](docs/FA3D/demo_full.mp4),
얼굴 전체를 3D 메쉬로 덮은 것은 [`docs/FA3D/demo_3d.mp4`](docs/FA3D/demo_3d.mp4) 입니다.

```bash
python eval/FA3D/infer.py --ckpt runs/fa3d/<런>/best.pth --source <video> \
    --corner3d 0.24 --max-faces 4 --drop-empty
```

| | |
|---|---|
| 검출 | 매 프레임 검출 — **얼굴이 없으면 아무것도 그리지 않습니다** |
| 추적 | 랜드마크로 다음 크롭을 잡되, 박스가 튀면 즉시 재검출 |
| 평활 | **62-d 파라미터에 한 번만** (랜드마크·메쉬가 같은 값에서 나오도록) |

> ⚠ **평활은 기본으로 꺼져 있습니다.** 논문 §2.4 가 시간 필터링은 *"정밀도를 깎고
> 프레임 지연을 만든다"* 며 반대하고, 그래서 저자는 후처리 대신 **svs 로 학습에서**
> 안정성을 얻었습니다. 데모 가독성을 위해 켤 때는(`--lmk-smooth`) 화면 상단에
> `smoothing` 이 표시되며, **평가에는 절대 쓰지 않습니다.**
...
> ⚠⚠ **이 NME 는 사실상 자세 지표입니다.** 3D 얼굴 형상을 얼마나 맞추는지는 거의 재지
> 않습니다 — 형상을 GT 로 완벽히 바꿔도 1.3% 개선뿐이고, 형상을 아예 안 내고 평균 얼굴을
> 써도 오히려 낫습니다. **따라서 위의 3.622 는 "자세를 재현했다"는 뜻입니다.**
> 형상 정확도는 `python scripts/FA3D/shape_accuracy.py` 로 따로 잽니다
...
68점을 이미지 위에 찍는 것만으로는 **z 를 볼 수 없습니다** — 3DDFA_V2 가 2D 정렬과
다른 점이 정확히 그 z 인데도 그렇습니다. 그래서 같은 예측을 **다른 각도에서 다시
렌더**했습니다(3~5열). 마지막 행(|yaw| 80°)은 원본에서 한쪽 뺨만 보이는데도 정면 얼굴이
복원됩니다.

> 3~5열은 평균 얼굴로부터의 identity 편차를 **×3 과장**하고 법선 방향 변위를
> 색칠했습니다(🔴 바깥 · 🔵 안쪽). BFM 의 shape 40-d 는 평균 얼굴에 얹는 **작은
> 섭동**이라, 있는 그대로 그리면 **GT 조차 사람마다 같아 보입니다.**

### ⚡ 추론 속도

`python scripts/FA3D/bench_backbone.py` — RTX 6000 Ada / batch 1, **추론 경로만**
(lrr 헤드도 synergy 순환도 이 저장소의 모델에는 아예 없습니다).

| 백본 | Params | MACs | CPU 1스레드 | GPU b1 | GPU b128 처리량 |
|---|---:|---:|---:|---:|---:|
| `mobilenet_v1` (논문 기본) | 3.3M | 177M | 6.12 ms | 1.78 ms | 23,162 img/s |
...
> 논문 보고치(CPU): MobileNet 6.2ms — 같은 자리입니다.
> ⚠ **파라미터 수는 지연시간의 대리 지표가 못 됩니다.** MobileNetV2 는 V1 보다
> 파라미터가 30% 적고 MACs 가 절반인데 CPU 는 같고 GPU 는 두 배 느립니다 —
> depthwise/inverted-residual 이 연산량이 아니라 메모리 대역폭에 묶입니다.
...
이 저장소는 한 걸음 더 가서 **모듈 자체를 만들지 않으므로**, 위 속도 표의 값이 곧
실제로 배포되는 모델의 값입니다.
