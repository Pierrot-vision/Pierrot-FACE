...
논문 원문·supplementary·전작(3DDFA v1) PDF: `<작업 루트>/3d_aligments/papers/`
...
> ### ⚠ "논문 설정 그대로" 가 성립하는 범위
>
> 논문 **supplementary §C** 가 실제로 밝힌 학습 설정은 여섯 개뿐이다 — 우리는 전부 맞췄다.
>
> | 항목 | 논문 | 우리 |
> |---|---|---|
> | 배치 (전역) | 128 | 128 ✅ |
> | 옵티마이저 | SGD · momentum 0.9 | 동일 ✅ |
> | weight decay | 0.0005 | 동일 ✅ |
> | meta-joint k | 100 | 100 ✅ |
> | svs 프레임 | 8 | 8 ✅ |
> | 입력 | 120×120 · (x−127.5)/128 | 동일 ✅ |
>
> **나머지는 논문에 없고 전부 우리 선택이다.**
>
> ```
> lr0 0.02 · cosine ×0.01 · warmup 1 epoch · 50 epoch
> clip_grad 1.0 · 비유한 손실 스킵
> nesterov=True            ← 논문은 "SGD with momentum" 이라고만 쓴다
> wd_scope='conv'          ← bias·norm 에는 wd 를 안 건다. 범위 미공개
> aux_mode='fixed' · aux_balance='share'(0.5) · lrr_weight 1.0
> ```
>
> ⚠⚠ **오피셜 학습 코드와는 대조가 불가능하다.** 저장소에는 추론부만 있다 —
> `models/mobilenet_v1.py` 는 62-d 단일 헤드뿐이고, 학습 루프·optimizer·손실 코드가 없다.
> 그래서 이 문서의 "논문 설정과 다른 점" 은 **우리 기준선 대비** 차이일 뿐이다.
>
> 참고 — M+R(보조항이 lrr 하나)에서 `aux_mode='ratio'`(논문 Algorithm 1 식)와
> 우리 `share(0.5)` 는 **수치적으로 동치**다 (`lrr_weight=1` 조건). 실측 확인:
> `share(0.5)=0.119251` vs `ratio=0.119251`, 차이 0. **ratio 실험은 불필요하다.**
> 다만 `lrr_weight≠1` 이거나 순환이 켜져 보조항이 6개면 갈린다.
...
`<작업 루트>/3d_aligments/` 에 함께 받아 둔 두 저장소와의 차이.
...
> ⚠ '기준 설정' 은 우리 기준선이지 논문 설정이 아니다 — 논문이 밝힌 것은 batch 128 ·
> SGD · momentum 0.9 · wd 5e-4 · k=100 · svs 8프레임 · 120x120 **여섯 개뿐**이고,
> lr·스케줄·epoch·warmup·clip·nesterov·wd 범위·보조손실 결합은 **전부 우리 선택**이다.
...
| 프리셋 | 기준 설정과 다른 점 | AFLW2000-3D | AFLW | shape std | ep |
|---|---|---|---|---|---|
| `meta_paper_aug_synergy_erase_idcons` | **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** ᵃ | 3.622 | 5.125 | — | 50 |
| `meta_paper_aug_synergy_occ` | **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** ᵃ | 3.624 | 5.120 | — | 50 |
| `meta_paper_aug_synergy_e80` | **epoch 50 → 80** · **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** ᵃ | 3.652 | 5.030 | — | 80 |
| `meta_paper_aug_synergy` | **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** ᵃ | 3.657 | 5.085 | — | 50 |
| `meta_paper_aug_synergy_erase02` | **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** ᵃ | 3.665 | 5.079 | — | 50 |
| `meta_paper_aug_synergy_erase` | **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** ᵃ | 3.669 | 5.173 | — | 50 |
| `meta_paper_aug_synergy_idcons` | **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** ᵃ | 3.678 | 5.093 | — | 50 |
| `meta_paper_aug` | **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** ᵃ | 3.688 | 5.094 | 0.588 | 50 |
| `meta_paper_aug_synergy_nonest` | **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** · **nesterov 끔** | 3.705 | 5.155 | — | 50 |
| `meta_paper_aug_synergy_s1` | **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** | 3.715 | 5.037 | — | 50 |
| `meta_paper_aug_synergy_pierrotx` | **백본 mobilenet_v1 → pierrotxv2_n** · **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** ᵃ | 3.733 | 5.222 | — | 50 |
| `meta_paper_synergy` | **순환(synergy) 켬** ᵃ | 3.758 | 5.154 | — | 50 |
| `meta_paper_aug_synergy_s2` | **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** | 3.763 | 5.238 | — | 50 |
| `meta_paper_idw` | 없음 (아래 기준 설정) ᵃ | 3.779 | 5.123 | — | 50 |
| `meta_paper_aug_synergy_yolo26` | **백본 mobilenet_v1 → yolo26_n** · **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** ᵃ | 3.783 | 5.246 | — | 50 |
| `meta_paper` | 없음 (아래 기준 설정) ᵃ | 3.793 | 5.102 | 0.657 | 50 |
| `meta_paper_x32` | **얼굴당 표본 0 → 32** ᵃ | 3.839 | 5.197 | — | 50 |
| `meta_synergy` | **배치 128 → 256** · **lr 0.02 → 0.04** · **순환(synergy) 켬** · **warmup(epoch) 1.0 → 2.0** ᵃ | 3.858 | 5.140 | 1.254 | 50 |
| `meta_paper_aug_synergy_pierrotx_lr` | **백본 mobilenet_v1 → pierrotxv2_n** · **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** · **warmup(epoch) 1.0 → 3.0** · **백본 LR 배수 1.0 → 0.1** | 3.860 | 5.358 | — | 50 |
| `meta_synergy_param` | **배치 128 → 256** · **lr 0.02 → 0.04** · **순환(synergy) 켬** · **param 손실 0.0 → 0.02** · **warmup(epoch) 1.0 → 2.0** ᵃ | 3.876 | 5.248 | — | 50 |
| `meta_paper_synergy_s02` | **순환(synergy) 켬** ᵃ | 3.987 | 5.396 | — | 50 |
| `meta_b256` | **배치 128 → 256** · **lr 0.02 → 0.04** · **warmup(epoch) 1.0 → 2.0** ᵃ | 4.099 | 5.530 | — | 50 |
| `mbv2` | **배치 128 → 256** · **lr 0.02 → 0.04** · **백본 mobilenet_v1 → mobilenet_v2** · **warmup(epoch) 1.0 → 2.0** ᵃ | 4.138 | 5.598 | — | 50 |
| `meta_wdall` | **배치 128 → 256** · **lr 0.02 → 0.04** · **weight decay 범위 conv → all** · **warmup(epoch) 1.0 → 2.0** ᵃ | 4.182 | 5.703 | — | 50 |
| `meta_paper_aug_synergy_idcons_v` | **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** ᵃ | *진행 중* | — | — | — |
...
- `meta_paper_aug_synergy_erase_idcons` — 당시 미도입: nesterov, 백본 LR 배수
- `meta_paper_aug_synergy_occ` — 당시 미도입: nesterov, 백본 LR 배수
- `meta_paper_aug_synergy_e80` — 당시 미도입: nesterov, 백본 LR 배수
- `meta_paper_aug_synergy` — 당시 미도입: nesterov, 백본 LR 배수
- `meta_paper_aug_synergy_erase02` — 당시 미도입: nesterov, 백본 LR 배수
- `meta_paper_aug_synergy_erase` — 당시 미도입: nesterov, 백본 LR 배수
- `meta_paper_aug_synergy_idcons` — 당시 미도입: nesterov, 백본 LR 배수
- `meta_paper_aug` — 당시 미도입: 얼굴당 표본, nesterov, 백본 LR 배수
- `meta_paper_aug_synergy_pierrotx` — 당시 미도입: nesterov, 백본 LR 배수
- `meta_paper_synergy` — 당시 미도입: nesterov, 백본 LR 배수
- `meta_paper_idw` — 당시 미도입: nesterov, 백본 LR 배수
- `meta_paper_aug_synergy_yolo26` — 당시 미도입: 백본 LR 배수
- `meta_paper` — 당시 미도입: 얼굴당 표본, nesterov, 백본 LR 배수
- `meta_paper_x32` — 당시 미도입: nesterov, 백본 LR 배수
- `meta_synergy` — 당시 미도입: 얼굴당 표본, nesterov, 백본 LR 배수
- `meta_synergy_param` — 당시 미도입: weight decay 범위, 얼굴당 표본, nesterov, 백본 LR 배수
- `meta_paper_synergy_s02` — 당시 미도입: nesterov, 백본 LR 배수
- `meta_b256` — 당시 미도입: param 손실, weight decay 범위, 색상 증강, 테두리 증강(px), 가림 증강, 얼굴당 표본, nesterov, 백본 LR 배수
- `mbv2` — 당시 미도입: param 손실, weight decay 범위, 색상 증강, 테두리 증강(px), 가림 증강, 얼굴당 표본, nesterov, 백본 LR 배수
- `meta_wdall` — 당시 미도입: 얼굴당 표본, nesterov, 백본 LR 배수
- `meta_paper_aug_synergy_idcons_v` — 당시 미도입: nesterov, 백본 LR 배수

#### `meta_paper_aug_synergy_erase_idcons` — erase + 자세 간 identity 일관성

- **런 디렉터리** — `runs/fa3d/meta_paper_aug_synergy_erase_idcons_meta_mobilenet_v1`
- **기준 설정과 다른 점** — **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01**  
  (당시 코드에 없던 옵션 — 꺼진 상태와 같다: nesterov, 백본 LR 배수)
...
- **결과** — AFLW2000-3D **3.624** · AFLW 5.120 (50 epoch)
- **읽을 때** — ⚠ **부분 성공.** AFLW2000-3D 는 3.657 → 3.624 로 좋아졌고 표적인 심한 가림(0~9점)에서 +1.295 → +0.949 로 **세 시도 중 최대 개선(-0.346)** 이다 (사각형 p=0.5 +0.140 · p=0.2 +0.175). **형태 가설은 맞았다.** 그런데 나머지 세 구간이 다시 나빠져 AFLW 총점은 5.085 → 5.120 으로 악화했다. 형태와 함께 빈도도 0.2 → 0.9 로 **동시에** 올린 탓이다 — 형태는 두고 빈도만 낮춘 `..._occ3`(0.3) 으로 가른다.

#### `meta_paper_aug_synergy_e80` — 학습 길이 50 → 80 epoch

- **런 디렉터리** — `runs/fa3d/meta_paper_aug_synergy_e80_meta_mobilenet_v1`
- **기준 설정과 다른 점** — **epoch 50 → 80** · **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01**  
  (당시 코드에 없던 옵션 — 꺼진 상태와 같다: nesterov, 백본 LR 배수)
- **왜 돌렸나** — 11개 런 중 6개가 ep47~50 에서 최고였다. 논문은 epoch 수·LR 스케줄을 안 밝혔고 우리 50 은 v1 코드에서 온 추정치다(SynergyNet 은 80). 미학습이면 지금까지의 모든 비교가 덜 학습된 상태끼리의 비교다.
- **결과** — AFLW2000-3D **3.652** · AFLW 5.030 (80 epoch)
- **읽을 때** — ❌ **미학습이 아니었다.** 3.652 vs 50ep 3.657 — **-0.005**, 잡음이다. ep61 에 3.697 에 닿은 뒤 20 epoch 정체했고, 중간 구간(ep41~49)에서는 코사인이 늘어난 탓에 오히려 +0.5~0.6 나빴다. 'ep47~50 최고'는 코사인 LR 이 바닥에 닿는 지점일 뿐 더 학습하면 내려간다는 뜻이 아니었다.
...
- **기준 설정과 다른 점** — **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01**  
  (당시 코드에 없던 옵션 — 꺼진 상태와 같다: 얼굴당 표본, nesterov, 백본 LR 배수)
...
#### `meta_paper_aug_synergy_nonest` — `nesterov=False`

- **런 디렉터리** — `runs/fa3d/meta_paper_aug_synergy_nonest_meta_mobilenet_v1`
- **기준 설정과 다른 점** — **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** · **nesterov 끔**
- **왜 돌렸나** — 논문은 "SGD with momentum" 이라고만 쓰고 nesterov 여부를 밝히지 않는다. 우리 `True` 는 선택이지 재현이 아니다. 오피셜 저장소에 학습 코드가 없어 대조도 불가능하므로, 남은 방법은 한 줄 바꿔 재는 것뿐이다.
- **결과** — AFLW2000-3D **3.705** · AFLW 5.155 (50 epoch)
- **읽을 때** — ❌ 기각 — 3.705 / 5.155 로 대조군(3.657 / 5.085) 대비 +0.048 / +0.070. ⚠ 다만 **판정 불가에 가깝다** — 단일 seed 이고 +0.048 은 우리가 아는 효과 크기 범위(±0.05~0.09) 안이다. seed 분산을 재기 전에는 이 판정에 근거가 없다.

#### `meta_paper_aug_synergy_s1` — 대조군과 **완전히 같은 설정**, seed 0 → 1

- **런 디렉터리** — `runs/fa3d/meta_paper_aug_synergy_s1_meta_mobilenet_v1`
- **기준 설정과 다른 점** — **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01**
- **왜 돌렸나** — 지금까지 20 런이 전부 seed 0 이다. 그런데 우리가 '실패'로 기각한 효과 크기가 e80 -0.005 · erase02 +0.008 · nonest +0.048 로 전부 작다. **seed 분산을 모르면 그 판정들은 판정이 아니라 추측이다.** 성능을 올리는 런이 아니라 자를 만드는 런.
- **결과** — AFLW2000-3D **3.715** · AFLW 5.037 (50 epoch)
- **읽을 때** — ⚠ **잡음이 우리가 잰 거의 모든 효과보다 크다.** 3.715 / 5.037 — 설정이 한 글자도 안 다른데 AFLW2000-3D 는 **+0.058 나쁘고 AFLW 는 -0.048 좋다**(두 벤치마크가 **반대 방향**). seed 2 까지 세 점을 모으면 **AFLW2000-3D 2σ ±0.106 · AFLW ±0.210** 이다 — 자세한 정정은 [Phase 12](Exp/Phase_12_잡음_기준선과_flip_TTA.md).

#### `meta_paper_aug_synergy_pierrotx` — 백본 PierrotXv2-n (obj365 사전학습)

- **런 디렉터리** — `runs/fa3d/meta_paper_aug_synergy_pierrotx_meta_pierrotxv2_n`
- **기준 설정과 다른 점** — **백본 mobilenet_v1 → pierrotxv2_n** · **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01**  
  (당시 코드에 없던 옵션 — 꺼진 상태와 같다: nesterov, 백본 LR 배수)
- **왜 돌렸나** — 지금까지 실패한 축은 전부 손실·증강·표집이었다. 300W-LP 는 얼굴 3,837개짜리 합성 셋이라 시각적 다양성이 없고, **obj365 검출 사전학습은 그 부족분을 가중치로 들여오는 유일한 경로**다. ⚠ 백본 축은 b256 시절 mbv2 한 번뿐이었고 최고 레시피에서는 처음이다. 파라미터 1.27M(현재의 39%)에 CPU 4.07ms(30% 빠름).
- **결과** — AFLW2000-3D **3.733** · AFLW 5.222 (50 epoch)
- **읽을 때** — ❌ **세 기준 모두 미달.** AFLW2000-3D 3.733(+0.076) · AFLW 5.222(+0.137) · 형상 설명분산 22%(-4%p). 속도 이점(30%)만 남았다. ⚠ 곡선은 사전학습이 **작동은 한다**고 말한다 — ep1 6.208 vs 대조군 15.678, ep25 4.145 vs 4.332 로 중반까지 앞서다가 ep49 에 +0.136 으로 역전당한다. 끝에서 뒤집히는 건 **용량 부족이나 레시피 불일치**의 모양이다. 학습 하이퍼파라미터는 하나도 안 바꿨고(lr0 0.02 · warmup 1 · 백본/헤드 LR 분리 없음), 파라미터도 동시에 3.27M → 1.27M 로 61% 줄어 두 원인이 **분리돼 있지 않다**.

#### `meta_paper_synergy` — 순환 단독 (논문 배치)

- **런 디렉터리** — `runs/fa3d/meta_paper_synergy_meta_mobilenet_v1`
- **기준 설정과 다른 점** — **순환(synergy) 켬**  
  (당시 코드에 없던 옵션 — 꺼진 상태와 같다: nesterov, 백본 LR 배수)
...
#### `meta_paper_aug_synergy_s2` — 대조군과 같은 설정, seed 0 → 2

- **런 디렉터리** — `runs/fa3d/meta_paper_aug_synergy_s2_meta_mobilenet_v1`
- **기준 설정과 다른 점** — **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01**
- **왜 돌렸나** — seed 0/1/2 로 잡음 폭을 확정한다.
- **결과** — AFLW2000-3D **3.763** · AFLW 5.238 (50 epoch)
- **읽을 때** — ⚠ **3.763 / 5.238 — 세 점 중 가장 나쁘다.** 세 점 종합: AFLW2000-3D 평균 3.712 (표준편차 0.053 · **2σ ±0.106**) · AFLW 평균 5.120(0.105 · **±0.210**). 두 점만 보고 추정했던 ±0.06 의 **두 배**다. 이 폭이면 백본 기각까지 흔들린다 — `pierrotx`(+0.076/+0.137)는 **판정 불가**다. ⚠ 그리고 **대조군 seed 평균 3.712 는 배포 가중치 3.683 보다 나쁘다** — '배포를 넘었다'는 운 좋은 한 런을 고른 말이다.

#### `meta_paper_idw` — identity 재가중 (얼굴 단위 ‖shape‖ 가중)

- **런 디렉터리** — `runs/fa3d/meta_paper_idw_meta_mobilenet_v1`
- **기준 설정과 다른 점** — 없음 (기준 그대로)  
  (당시 코드에 없던 옵션 — 꺼진 상태와 같다: nesterov, 백본 LR 배수)
...
#### `meta_paper_aug_synergy_yolo26` — 백본 YOLO26-n (obj365 오피셜 사전학습)

- **런 디렉터리** — `runs/fa3d/meta_paper_aug_synergy_yolo26_meta_yolo26_n`
- **기준 설정과 다른 점** — **백본 mobilenet_v1 → yolo26_n** · **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01**  
  (당시 코드에 없던 옵션 — 꺼진 상태와 같다: 백본 LR 배수)
- **왜 돌렸나** — PierrotXv2-n 과 같은 축의 두 번째 후보. 차이는 가중치 출처다 — 이쪽은 **오피셜 obj365 체크포인트**(yolo26n-objv1-150 · ultralytics 8.3.222 · 학습 1,742,289장)를 그대로 쓴다. 1.37M · MACs 61M · CPU 4.05ms. 백본 축을 한 번의 실패로 닫지 않기 위해 같은 조건에서 한 번 더 잰다.
- **결과** — AFLW2000-3D **3.783** · AFLW 5.246 (50 epoch)
- **읽을 때** — ❌ **기각 — PierrotXv2-n 보다도 나쁘다.** AFLW2000-3D 3.783(+0.126) · AFLW 5.246(+0.161). ⚠ 초반 9 epoch 은 PierrotXv2-n 을 평균 -0.375 로 앞섰는데(대조군 상대로는 -1.787) **최종은 뒤집혔다** — 사전학습 백본의 초반 우위는 최종 성능을 전혀 예측하지 못한다. 예측했던 ep26~30 붕괴도 이쪽엔 없었다(그건 PierrotXv2-n 개체 현상). 붕괴 없이 그냥 못 따라잡는 모양이라 원인은 레시피보다 **용량(3.27M → 1.37M)** 쪽을 가리킨다.

#### `meta_paper` — 논문 명시 항목 준수 (M+R)
...
- **기준 설정과 다른 점** — **배치 128 → 256** · **lr 0.02 → 0.04** · **순환(synergy) 켬** · **warmup(epoch) 1.0 → 2.0**  
  (당시 코드에 없던 옵션 — 꺼진 상태와 같다: 얼굴당 표본, nesterov, 백본 LR 배수)
...
#### `meta_paper_aug_synergy_pierrotx_lr` — 사전학습 백본 전용 레시피 — warmup 3 + 백본 `lr_mult` 0.1

- **런 디렉터리** — `runs/fa3d/meta_paper_aug_synergy_pierrotx_lr_meta_pierrotxv2_n`
- **기준 설정과 다른 점** — **백본 mobilenet_v1 → pierrotxv2_n** · **순환(synergy) 켬** · **색상 증강 0.0 → 0.4** · **테두리 증강(px) 0 → 5** · **가림 증강 0.0 → 0.01** · **warmup(epoch) 1.0 → 3.0** · **백본 LR 배수 1.0 → 0.1**
- **왜 돌렸나** — `pierrotx` 곡선이 ep26~30 에 4.145 → 5.036 으로 **튀어 올랐다**. 코사인 LR 이 아직 높은 절반 지점(lr~0.011)이라 **사전학습 백본이 그 LR 을 못 견딘다**는 진단을 세웠다. 논문이 밝힌 값(momentum 0.9)과 우리가 실측으로 고른 값(lr0 0.02)은 두고, **공개되지 않아 우리 선택인 두 축만** 바꾼다 — warmup 1→3, 백본 실효 LR 0.02→0.002. `engine.py` 가 지원하면서 아무도 안 쓰던 `lr_mult` 를 처음 배선했다.
- **결과** — AFLW2000-3D **3.860** · AFLW 5.358 (50 epoch)
- **읽을 때** — ❌ **기각 — 트레이드오프이지 개선이 아니다.** 3.860 / 5.358 로 기존 `pierrotx`(3.733 / 5.222)보다 세 지표 다 나쁘다(재주석 +0.215 로 최대). ✅ **진단은 맞았다** — ep26~40 에서 기존 대비 평균 -0.217(12/15 승), ep29 붕괴가 5.036 → 3.955 로 **사라졌다**. ❌ 대신 **ep41~50 을 0/10 전패**했다. 백본 LR 을 1/10 로 묶으니 코사인이 바닥에 닿는 구간에서 미세조정이 불가능해져, ep33 에 3.900 을 찍고 남은 17 epoch 을 3.86~4.02 에서 맴돈다. **붕괴를 막은 대가로 최종 수렴을 잃었다.** 가설 C 기각 — 이 백본의 한계는 레시피가 아니라 **용량**이다.

#### `meta_synergy_param` — 순환 + 파라미터 공간 손실 (B=256)

- **런 디렉터리** — `runs/fa3d/meta_synergy_param_meta_mobilenet_v1_b256`
- **기준 설정과 다른 점** — **배치 128 → 256** · **lr 0.02 → 0.04** · **순환(synergy) 켬** · **param 손실 0.0 → 0.02** · **warmup(epoch) 1.0 → 2.0**  
  (당시 코드에 없던 옵션 — 꺼진 상태와 같다: weight decay 범위, 얼굴당 표본, nesterov, 백본 LR 배수)
...
- **기준 설정과 다른 점** — **배치 128 → 256** · **lr 0.02 → 0.04** · **warmup(epoch) 1.0 → 2.0**  
  (당시 코드에 없던 옵션 — 꺼진 상태와 같다: param 손실, weight decay 범위, 색상 증강, 테두리 증강(px), 가림 증강, 얼굴당 표본, nesterov, 백본 LR 배수)
...
- **기준 설정과 다른 점** — **배치 128 → 256** · **lr 0.02 → 0.04** · **백본 mobilenet_v1 → mobilenet_v2** · **warmup(epoch) 1.0 → 2.0**  
  (당시 코드에 없던 옵션 — 꺼진 상태와 같다: param 손실, weight decay 범위, 색상 증강, 테두리 증강(px), 가림 증강, 얼굴당 표본, nesterov, 백본 LR 배수)
...
- **기준 설정과 다른 점** — **배치 128 → 256** · **lr 0.02 → 0.04** · **weight decay 범위 conv → all** · **warmup(epoch) 1.0 → 2.0**  
  (당시 코드에 없던 옵션 — 꺼진 상태와 같다: 얼굴당 표본, nesterov, 백본 LR 배수)
